...
export const DynamicDashboardPage: React.FC<DynamicDashboardPageProps> = ({
  ...
  const getTimeframeStart = (filter: string): number | null => {
    const now = Date.now();
    ...
    if (filter === '1y') return now - 365 * 24 * 60 * 60 * 1000;

    return null;
  };

  const filterWidgetRowsByTimeframe = (
    rows: Record<string, any>[]
  ): Record<string, any>[] => {
    if (!rows.length || timeframeFilter === 'all') {
      return rows;
    }

    const dateColumn = Object.keys(rows[0]).find((column) =>
      /(date|time|timestamp|created|updated|modified|closed|opened|start|end)/i.test(
        column
      )
    );
    ...
    const start = getTimeframeStart(timeframeFilter);
  ...
  const loadDashboardVersions = async () => {
    ...
    try {
      const versions = await copilotApi.getDashboardVersions(activeDashboardId);
      setDashboardVersions(versions);
    } catch (err: any) {
      showToast(
        'error',
        'Version History Failed',
        err?.message || 'Could not load dashboard version history.'
      );
    } finally {
  ...
  const handleExecutePrompt = async (promptText: string) => {
    ...
    try {
      ...
      // Natural-language dashboard modification: add/remove/change widgets.
      // Keep ordinary analytics questions on the existing query pipeline.
      const isDashboardModification = /^(add|remove|delete|change|modify|convert|make)\b.*\b(dashboard|widget|chart|kpi|visualization)\b/i.test(
        promptText.trim()
      ) || /\b(remove|delete|change|modify|convert)\b.*\b(chart|widget|kpi|visualization)\b/i.test(
        promptText.trim()
      );

      if (isDashboardModification) {
        if (!activeDashboardId) {
          throw new Error('No active dashboard is available for modification.');
        }
        ...
        if (isNaN(numericSourceId)) {
          throw new Error('No valid connected source is selected.');
        }

        const modifyResult = await copilotApi.modifyDashboard({
          dashboard_id: activeDashboardId,
          instruction: promptText.trim(),
          source_id: numericSourceId,
        });

        const modificationMsg: ChatMessage = {
          id: `msg_modify_${Date.now()}`,
          sender: 'assistant',
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          content: modifyResult.success
            ? modifyResult.message
            : modifyResult.error || modifyResult.message || 'Dashboard modification could not be completed.',
          sqlQuery: modifyResult.sql || undefined,
        };

        setMessages((prev) => [...prev, modificationMsg]);

        if (!modifyResult.success) {
          showToast('error', 'Dashboard Modification Failed', modifyResult.error || modifyResult.message);
          setIsProcessing(false);
          return;
        }

        // Refresh persisted dashboard widgets from the backend so the UI reflects
        // the actual database state after the modification.
        const refreshedWidgets = await copilotApi.getWidgets(activeDashboardId);
        setPersistedWidgets(refreshedWidgets);

        if (modifyResult.action_taken === 'remove' && modifyResult.widget_id) {
          setWidgetDataMap((prev) => {
            const next = { ...prev };
            delete next[modifyResult.widget_id as number];
            return next;
          });
        }
        ...
        showToast(
          'success',
          'Dashboard Updated',
          modifyResult.message || 'Dashboard modification completed successfully.'
        );
  ...
  return (
    <div ref={pageContainerRef} data-pi-dashboard-export-root="true" className="space-y-6">
      {/* Top Header */}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {persistedWidgets.map((w) => {
                  ...
                  const filteredRows = cached
                    ? filterWidgetRowsByTimeframe(cached.rows)
                    : [];
            ...
            {dashboardState.kpis.map((kpi, idx) => {
              ...
              return (
                <EnhancedKPICard
                  ...
                  onDrilldown={() => onNavigate('explorer', { sourceId: selectedSourceId })}
                />
              );
            })}
          </div>
      ...
      <DashboardExportModal
        ...
        exportData={{
          ...
          systemMetrics: {
            connectedSources: allSources.length,
            catalogedTables: allSources.reduce((acc, s) => acc + (s.tableCount || 0), 0),
            syncFreshness: allSources.some((s) => s.lastSyncAt && s.lastSyncAt !== 'Never') ? 'Available' : 'Not available',
            systemHealth: allSources.some((s) => s.status === 'healthy') ? 'Healthy' : 'No healthy source',
          },
          ...
          widgets: (() => {
            const persisted = persistedWidgets.map((w) => ({
              id: w.id,
              title: w.title,
              widget_type: w.widget_type,
              sql_query: w.sql_query,
              visualization_spec: w.visualization_spec,
              columns: widgetDataMap[w.id]?.columns || [],
              rows: widgetDataMap[w.id]?.rows || [],
            }));

            // Export the exact latest Copilot result currently shown in the
            // chat, including its visualization spec and real query rows.
            const latestCopilot = [...messages]
              .reverse()
              .find(
                (m) =>
                  m.sender === 'assistant' &&
            ...
            if (latestCopilot?.visualization) {
              persisted.push({
                id: -1,
                title:
                  latestCopilot.visualization.title ||
                  'Copilot Visualization',
                widget_type:
                  latestCopilot.visualization.type || 'bar',
                sql_query: latestCopilot.sqlQuery,
                visualization_spec: latestCopilot.visualization,
                columns: latestCopilot.columns || [],
                rows: latestCopilot.rows || [],
              });
            }

            return persisted;
          })(),
          dataTable: dashboardState.dataTable
            ? {
                title: dashboardState.dataTable.title,
                columns: dashboardState.dataTable.columns.map((c) => ({
                  key: c.key,
                  header: c.header,
                })),
                rows: dashboardState.dataTable.rows,
              }
            : undefined,
        }}
      />
