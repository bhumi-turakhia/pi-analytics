...
export const ActivityPage: React.FC<{ onNavigate: (page: NavigationPage) => void }> = () => {
  ...
  const loadReportHistory = async () => {
    ...
    try {
      const data = await activityApi.getReportHistory();
  ...
  const mapStatusForReport = (status: string): ActivityLog['status'] => {
  ...
  const mapReportStatus = (status: string): ActivityLog['status'] => {
    return mapStatusForReport(status);
  };
  return (
