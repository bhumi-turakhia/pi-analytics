        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              ...
              <Input
                label={platform === 'salesforce' ? 'OAuth Authorization Service' : 'Virtual Warehouse'}
                placeholder={platform === 'salesforce' ? 'Salesforce OAuth' : 'Snowflake warehouse'}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              <Input
                label={platform === 'salesforce' ? 'API Scope' : 'Database'}
                placeholder={platform === 'salesforce' ? 'api' : 'RETAIL_ANALYTICS'}
                value={formData.database}
                onChange={(e) => setFormData({ ...formData, database: e.target.value })}
              />

              <Input
                label={platform === 'salesforce' ? 'API Version' : 'Default Schema'}
                placeholder={platform === 'salesforce' ? 'Auto-detected' : 'SALES'}
                value={formData.defaultSchema}
                onChange={(e) => setFormData({ ...formData, defaultSchema: e.target.value })}
                helperText={platform === 'salesforce' ? 'Salesforce API version is discovered automatically when left blank.' : undefined}
              />
            </div>
            ...
            <div className="pt-2 border-t border-neutral-200">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-neutral-500 font-medium">
                  {platform === 'salesforce'
                    ? 'Verify Salesforce OAuth authorization and API access'
                  ...
                  {testResult.details?.snowflakeVersion && (
                    <div className="mt-2 pt-2 border-t border-emerald-200 text-[11px] font-mono-code text-emerald-900 flex items-center gap-3">
                      ...
                      {testResult.details.warehouseStatus && (
                        <span>â€¢ WH: {testResult.details.warehouseStatus}</span>
                      )}
                    </div>
                  )}
                </div>
