...
export const DataSourcesPage: React.FC<DataSourcesPageProps> = ({
  ...
  const promptSyncMetadata = (source: DataSource, e?: React.MouseEvent) => {
    ...
    const isSalesforce = source.platform === 'salesforce';
    ...
    setSyncCredentials({
      accountIdentifier: isSalesforce
        ? 'https://orgfarm-a6c098ac-b4-dev-ed.develop.my.salesforce.com'
      ...
      database: isSalesforce
        ...
        : source.database || '',
      username: '',
      role: isSalesforce ? 'api' : source.role || '',
      password: '',
    });
  ...
  const handleExecuteMetadataSync = async () => {
    ...
    try {
      const isSalesforce = sourceForMetadataSync.platform === 'salesforce';
      ...
      if (isSalesforce) {
        payload.account_identifier =
          syncCredentials.accountIdentifier.trim();
        payload.warehouse = 'Salesforce OAuth';
        payload.database = 'api';
        payload.role = 'api';
      }
  ...
  const handleExecuteTestConnection = async () => {
    ...
    // Validate credentials according to the selected source platform.
    const missing: string[] = [];

    if (sourceForTest.platform === 'salesforce') {
      if (!testCredentials.salesforceLoginUrl.trim()) {
        missing.push('Salesforce Login URL');
      }
      if (!testCredentials.salesforceClientId.trim()) {
        missing.push('Client ID');
      }
      if (!testCredentials.salesforceClientSecret.trim()) {
        missing.push('Client Secret');
      }

      if (missing.length > 0) {
        setTestValidationError(
          `Please supply required Salesforce credentials: ${missing.join(', ')}.`
        );
        return;
      }
    } else {
      if (!testCredentials.accountIdentifier.trim()) {
        missing.push('Account Identifier');
      }
      if (!testCredentials.username.trim()) {
        missing.push('Username');
      }
      if (!testCredentials.password.trim()) {
        missing.push('Password');
      }

      if (missing.length > 0) {
        setTestValidationError(
          `Please supply required Snowflake credentials: ${missing.join(', ')}.`
        );
        return;
      }
    }
    ...
    try {
      const result = await dataSourceApi.testConnection({
        id: sourceForTest.id,
        platform: sourceForTest.platform,
        ...(sourceForTest.platform === 'salesforce'
          ? {
              salesforceLoginUrl: testCredentials.salesforceLoginUrl.trim(),
              salesforceClientId: testCredentials.salesforceClientId.trim(),
              salesforceClientSecret: testCredentials.salesforceClientSecret,
            }
          : {
      ...
      const errMsg =
        err?.message ||
        `Failed to verify ${sourceForTest.platform === 'salesforce' ? 'Salesforce' : 'Snowflake'} connection.`;
      ...
      // Immediately clear sensitive credentials from state.
  ...
  return (
    <div className="space-y-6">
                  ...
                  {!isDatabricks ? (
                    <>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {(source.platform === 'snowflake' || source.platform === 'salesforce') && (
                          <Button
                            size="sm"
                            variant="outline"
                            disabled={syncingId !== null || isTestingConnection || disconnectingId !== null || isSyncingMetadata}
        ...
        {sourceForMetadataSync && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-neutral-600 leading-relaxed">
              {sourceForMetadataSync.platform === 'salesforce'
                ? <>Discovers Salesforce objects and field definitions through the Salesforce metadata API. Business data rows are never queried or ingested.</>
              ...
              <>
                <div className="space-y-3 pt-1">
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <label className="block text-[11px] font-bold text-neutral-700 uppercase mb-1">
                        {sourceForMetadataSync.platform === 'salesforce' ? 'Salesforce My Domain' : 'Account Identifier'}
                      </label>
                      <input
                        type="text"
                        value={syncCredentials.accountIdentifier}
                        onChange={(e) => setSyncCredentials({ ...syncCredentials, accountIdentifier: e.target.value })}
                        placeholder={sourceForMetadataSync.platform === 'salesforce'
                          ? 'https://your-domain.my.salesforce.com'
                          : 'e.g. xy94821.us-east-1'}
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-neutral-700 uppercase mb-1">
                        {sourceForMetadataSync.platform === 'salesforce' ? 'OAuth Authorization Service' : 'Warehouse'}
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <label className="block text-[11px] font-bold text-neutral-700 uppercase mb-1">
                        {sourceForMetadataSync.platform === 'salesforce' ? 'API Scope' : 'Database'}
                      </label>
                      <input
                        type="text"
                        ...
                        placeholder={sourceForMetadataSync.platform === 'salesforce'
                          ? 'api'
                          : 'e.g. RETAIL_ANALYTICS'}
                        className="w-full text-xs font-mono-code bg-white border border-neutral-300 rounded px-2.5 py-1.5 focus:border-black focus:outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-neutral-700 uppercase mb-1">
                        {sourceForMetadataSync.platform === 'salesforce' ? 'OAuth Client ID' : 'Username'}
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <label className="block text-[11px] font-bold text-neutral-700 uppercase mb-1">
                        {sourceForMetadataSync.platform === 'salesforce' ? 'OAuth Scope' : 'Role (Optional)'}
                      </label>
                      <input
                        type="text"
                ...
                <div className="p-3 rounded-md bg-stone-50 border border-stone-200 text-stone-700 text-xs flex items-start gap-2">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-stone-600 leading-normal">
                      Credentials are sent via encrypted TLS, used ephemerally in-memory to discover metadata, and never stored in PostgreSQL or logged.
                    </p>
                  </div>
                </div>
      ...
      <Modal
        ...
        subtitle={
          sourceForTest?.platform === 'salesforce'
            ? 'Verify Salesforce OAuth authentication and API access'
            : 'Verify live SSL handshake, authentication, and warehouse authorization'
        }
        maxWidth="lg"
        footer={
          <>
      ...
      >
        {sourceForTest && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-neutral-600 leading-relaxed">
              {sourceForTest.platform === 'salesforce'
                ? 'Uses the backend-configured Salesforce OAuth Client Credentials to verify authentication and API access. Salesforce secrets remain server-side and are never exposed to the browser.'
            ...
            {sourceForTest.platform === 'salesforce' ? (
              <div className="space-y-3 pt-1">
                <div>
                  <label className="block text-[11px] font-bold text-neutral-700 uppercase mb-1">
                    Salesforce Login URL <span className="text-rose-600">*</span>
                  </label>
                  <input
                    type="text"
                    value={testCredentials.salesforceLoginUrl}
                    onChange={(e) => {
                      setTestCredentials({
                        ...testCredentials,
                        salesforceLoginUrl: e.target.value,
                      });
                      setTestValidationError(null);
                    }}
                    placeholder="https://login.salesforce.com"
                    className="w-full px-3 py-2.5 rounded-lg border border-neutral-300 bg-white text-xs text-black focus:outline-none focus:ring-2 focus:ring-black/10"
                  />
                  <p className="mt-1 text-[10px] text-neutral-500">
                    Use your Salesforce login URL or My Domain URL.
                  </p>
                </div>

                <div>
                  <label className="block text-[11px] font-bold text-neutral-700 uppercase mb-1">
                    Client ID <span className="text-rose-600">*</span>
                  </label>
                  <input
                    type="text"
                    value={testCredentials.salesforceClientId}
                    onChange={(e) => {
                      setTestCredentials({
                        ...testCredentials,
                        salesforceClientId: e.target.value,
                      });
                      setTestValidationError(null);
                    }}
                ...
                <div>
                  <label className="block text-[11px] font-bold text-neutral-700 uppercase mb-1">
                    Client Secret <span className="text-rose-600">*</span>
                  </label>
                  <input
                    type="password"
                    value={testCredentials.salesforceClientSecret}
                    onChange={(e) => {
                      setTestCredentials({
                        ...testCredentials,
                        salesforceClientSecret: e.target.value,
                      });
                      setTestValidationError(null);
                    }}
                ...
                <div className="p-3 rounded-lg bg-neutral-50 border border-neutral-200 text-[10px] text-neutral-600 leading-relaxed">
                  <span className="font-bold text-neutral-800">
                    Zero-persistence credential security:
                  </span>{' '}
                  these credentials are used only for this connection test and
                  are not persisted in PostgreSQL, localStorage, or application logs.
                </div>
              </div>
            ) : (
              <div className="space-y-3 pt-1">
            ...
            <div className="p-3 rounded-md bg-stone-50 border border-stone-200 text-stone-700 text-xs flex items-start gap-2">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] text-stone-600 leading-normal">
                  {sourceForTest?.platform === 'salesforce'
                    ? 'Salesforce OAuth secrets remain server-side and are never exposed to the browser or stored in PostgreSQL, localStorage, or logs.'
