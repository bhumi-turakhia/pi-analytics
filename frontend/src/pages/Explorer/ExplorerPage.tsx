...
export const ExplorerPage: React.FC<ExplorerPageProps> = ({ onNavigate, initialContext }) => {
  ...
  const selectedSource = useMemo(
    () => sources.find((s) => String(s.id) === selectedSourceId),
    [sources, selectedSourceId]
  );

  const selectedSourceType = (selectedSource?.platform || 'snowflake').toLowerCase();

  const buildPreviewSql = (db: string, sch: string, tbl: string, limit: number) => {
    if (!tbl) return '-- Select a table from the connected catalog to preview.';

    if (selectedSourceType === 'salesforce') {
      return `SELECT FIELDS(ALL) FROM ${tbl} LIMIT ${limit}`;
    }
  ...
  useEffect(() => {
    ...
    async function loadCatalog() {
      ...
      try {
        ...
        const firstDataset = relevantDatasets.length > 0 ? relevantDatasets[0] : null;
        const initDb = initialContext?.database || firstDataset?.database_name || '';
        const initSch = initialContext?.schema || firstDataset?.schema_name || '';
        const initTbl = initialContext?.table || firstDataset?.table_name || '';

        setDatabase(initDb);
        setSchema(initSch);
        setTable(initTbl);

        const defaultPreviewSql = buildPreviewSql(initDb, initSch, initTbl, 50);
  ...
  const availableDatabases = useMemo(() => {
    const filtered = selectedSourceId
      ...
      : datasets;
    return Array.from(new Set(filtered.map((d) => d.database_name))).filter(Boolean);
  ...
  const availableSchemas = useMemo(() => {
    ...
    return Array.from(new Set(filtered.map((d) => d.schema_name))).filter(Boolean);
  ...
  const availableTables = useMemo(() => {
    ...
    return Array.from(new Set(filtered.map((d) => d.table_name))).filter(Boolean);
  ...
  const handleDatabaseChange = (newDb: string) => {
    ...
    const newSchema = validSchemas[0] || '';
    setSchema(newSchema);
    ...
    const validTables = datasets
      ...
      .map((d) => d.table_name);
    const newTable = validTables[0] || '';
    setTable(newTable);
    if (newDb && newSchema && newTable) {
      setSqlQuery(buildPreviewSql(newDb, newSchema, newTable, rowLimit));
    }
  };
  ...
  const handleSchemaChange = (newSchema: string) => {
    ...
    const newTable = validTables[0] || '';
    setTable(newTable);
    if (database && newSchema && newTable) {
      setSqlQuery(buildPreviewSql(database, newSchema, newTable, rowLimit));
    }
  };

  const handleTableChange = (newTable: string) => {
    setTable(newTable);
    if (database && schema && newTable) {
      setSqlQuery(buildPreviewSql(database, schema, newTable, rowLimit));
    }
  };
              ...
              onClick={() =>
                handlePresetSelect(
                  database && schema && table
                    ? `SELECT *\nFROM "${database}"."${schema}"."${table}"\nLIMIT ${rowLimit};`
                    : `SELECT *\nFROM "PI_ANALYTICS"."ANALYTICS"."SALES"\nLIMIT 50;`
                )
              }
