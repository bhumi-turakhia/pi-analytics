...
export const DashboardExportModal: React.FC<DashboardExportModalProps> = ({
  ...
  const handleExecuteExport = async () => {
    ...
    const options: ExportOptions = {
      ...
      includeMetadata,
      includeBranding,
      filename: `pi-analytics-dashboard-${selectedResolution}-${Date.now()}.${selectedFormat === 'jpeg' ? 'jpg' : selectedFormat}`,
    };
    ...
    try {
      ...
      showToast('success', 'Export Completed', `Dashboard saved successfully as ${selectedFormat.toUpperCase()} (${selectedResolution}).`);
      onClose();
    } catch (error: any) {
