...
def _find_best_table_and_columns(question: str, catalog):
    ...
    if not scored_tables or scored_tables[0][0] <= 0:
        return None
...
def generate_catalog_grounded_sql(question: str, catalog, limit: int = 100) -> Tuple[str, Dict[str, Any]]:
    ...
    # Explicit record/data requests always return raw rows as a table.
    is_record_request = any(k in q_lower for k in (
        "show records", "show me the records",
        "show rows", "show me the rows",
        "show data", "show me the data",
        "show details", "show me the details",
        "list records", "list rows",
        "raw records", "raw data"
    ))

    # Count-by-dimension requests require GROUP BY.
    is_count_breakdown = is_count_intent and any(k in q_lower for k in (
        "by ", "per ", "across ", "each ", "breakdown", "group by"
    ))
    is_aggregate_only = (
    ...
    # Explicit raw-record intent must always return a table.
...
def generate_catalog_grounded_soql(question: str, catalog, limit: int = 100) -> Tuple[str, Dict[str, Any]]:
    """
    Deterministically generate read-only Salesforce SOQL from the runtime catalog.
    Uses the same catalog-driven intent heuristics as the SQL fallback, but emits
    Salesforce-compatible SOQL syntax.
    """
    best = _find_best_table_and_columns(question, catalog)
    if not best:
        raise ValueError("No matching catalog table found.")
    ...
    numeric_cols = cols.get("numeric", [])
    category_cols = cols.get("category", [])
    id_cols = cols.get("id", [])

    def choose_category() -> Optional[str]:
        candidates = category_cols + id_cols
        if not candidates:
            return None
        return _find_best_category_col(question, candidates)

    def choose_numeric() -> Optional[str]:
        return _find_best_numeric_col(question, numeric_cols)

    # Raw-record questions must bypass metric aggregation.
    is_raw_records = any(
        phrase in q_lower
        for phrase in (
            "show me the records",
    ...
    if is_raw_records:
        selected = [c["name"] for c in tbl.get("columns", [])[:10]]
        if not selected:
            selected = ["Id"]

        return (
            f"SELECT {', '.join(selected)} FROM {table_name} LIMIT {bounded_limit}",
            {
                "type": "table",
    ...
    is_ranking = ranking_match is not None or any(
        word in q_lower
        for word in ("top", "bottom", "highest", "lowest", "ranking", "ranked")
    )

    if is_count and is_ranking:
        n = int(ranking_match.group(1)) if ranking_match else 10
        cat = choose_category()
        ...
        if not cat:
            return (
                ...
                {
                    "type": "kpi",
                    "title": f"Total {table_name} Count",
                    "xField": None,
                    "yField": None,
                },
            )
        ...
        soql = (
            f"SELECT {cat}, COUNT(Id) "
            f"FROM {table_name} "
            f"GROUP BY {cat} "
            f"ORDER BY COUNT(Id) DESC "
            f"LIMIT {min(n, bounded_limit)}"
        )
        return soql, {
            "type": "bar",
            "title": f"Top {n} {cat.replace('__c', '').replace('_', ' ').title()} by Count",
            "xField": cat,
            "yField": "expr0",
        }
    ...
    if is_count:
        ...
        if cat and any(
            phrase in q_lower
            for phrase in ("by ", "per ", "across ", "each ", "group", "region")
        ):
            soql = (
                f"SELECT {cat}, COUNT(Id) "
                f"FROM {table_name} "
            ...
            return soql, {
                "type": "bar",
                "title": f"Count by {cat.replace('__c', '').replace('_', ' ').title()}",
                "xField": cat,
                "yField": "expr0",
            }
    ...
    # Sum/average/min/max by a dimension.
    numeric_col = choose_numeric()

    if numeric_col:
        if any(word in q_lower for word in ("average", "avg", "mean")):
            agg = "AVG"
        elif any(word in q_lower for word in ("minimum", "minimum", "min")):
            agg = "MIN"
        elif any(word in q_lower for word in ("maximum", "max")):
            agg = "MAX"
        else:
            agg = "SUM"
        ...
        if cat and wants_breakdown:
            soql = (
                f"SELECT {cat}, {agg}({numeric_col}) "
                f"FROM {table_name} "
                f"GROUP BY {cat} "
                f"ORDER BY {agg}({numeric_col}) DESC "
                f"LIMIT {bounded_limit}"
            )
            return soql, {
                "type": "bar",
                "title": f"{agg.title()} {numeric_col.replace('__c', '').replace('_', ' ').title()} by {cat.replace('__c', '').replace('_', ' ').title()}",
                "xField": cat,
                "yField": "expr0",
            }

        return f"SELECT {agg}({numeric_col}) FROM {table_name}", {
            "type": "kpi",
            "title": f"{agg.title()} {numeric_col.replace('__c', '').replace('_', ' ').title()}",
            "xField": None,
            "yField": "expr0",
        }

    # Safe raw-record fallback.
    selected = [c["name"] for c in tbl.get("columns", [])[:10]]
    if not selected:
        selected = ["Id"]
...
def copilot_query(payload: CopilotQueryRequest):
    ...
    # 6. Generate a source-specific read-only query.
    #
    # Salesforce uses SOQL and must not receive SQL generated for Snowflake.
    # Snowflake retains the existing Gemini + deterministic SQL pipeline.
    if source_type == "salesforce":
        try:
            generated_sql, viz_intent = generate_catalog_grounded_soql(
                payload.question,
                catalog,
                limit=payload.limit or 100,
            )
            explanation = f"Generated analytical SOQL query for: {payload.question}"
        except Exception as e:
            _log_audit(
                action_type="copilot_query",
                status="failure",
            ...
            return CopilotQueryResponse(
                success=False,
                answer=f"Could not generate Salesforce query for this question: {str(e)}",
                error=str(e),
            )
    else:
        # Existing Snowflake path remains unchanged.
    ...
    # 7. Safety validation.
    # Snowflake uses the existing SQL safety validator.
    # SalesforceConnector performs its own strict read-only SOQL validation.
    if source_type != "salesforce":
        try:
