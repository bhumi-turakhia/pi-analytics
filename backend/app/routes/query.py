...
def execute_query(payload: QueryExecutePayload):
    ...
    if source_type != "snowflake":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Unsupported data source type '{source_type}'. Real query execution is currently supported for Snowflake."
        )
    ...
    if payload.sql_query and payload.sql_query.strip():
        ...
        # Construct safe preview query with double-quoted identifiers
