...
def delete_source(source_id: int):
    """
    Safely delete a data source and all its dependent application metadata records.

    Deletion order (preserves referential integrity, never touches Snowflake data):
      1. catalog_columns   – deleted via CASCADE on datasets FK
      2. datasets          – explicitly deleted before data_sources
      3. pipeline_runs     – explicitly deleted before data_sources
      4. data_sources      – deleted last

    dashboards.source_id and dashboard_widgets.source_id are set to NULL
    automatically via ON DELETE SET NULL on those foreign keys.
    """
    with engine.begin() as connection:
        # 1. Validate that the source exists
        source_result = connection.execute(
        ...
        # 2. Count dependents so we can report what was cleaned up
        dataset_count = connection.execute(
            text("SELECT COUNT(*) FROM datasets WHERE source_id = :source_id"),
            {"source_id": source_id}
        ).scalar() or 0
        ...
        # 3. Count catalog columns that will be removed (cascade via dataset deletion)
        column_count = connection.execute(
            text("""
                SELECT COUNT(*) FROM catalog_columns cc
                JOIN datasets d ON cc.dataset_id = d.id
                WHERE d.source_id = :source_id
            """),
            {"source_id": source_id}
        ).scalar() or 0

        # 4. Remove dependents in safe dependency order:
        #    catalog_columns are deleted automatically via ON DELETE CASCADE on datasets.id
        #    so we only need to delete datasets and pipeline_runs explicitly.
        try:
            # Step 4a: Delete datasets (catalog_columns cascade automatically)
            connection.execute(
                text("DELETE FROM datasets WHERE source_id = :source_id"),
                {"source_id": source_id}
            )

            # Step 4b: Delete pipeline runs
            connection.execute(
                text("DELETE FROM pipeline_runs WHERE source_id = :source_id"),
                {"source_id": source_id}
            )

            # Step 4c: Delete the data source itself
            #          dashboards/dashboard_widgets use ON DELETE SET NULL, so no action needed
            connection.execute(
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to delete data source {source_id}: {str(e)}"
            )
        ...
        return {
            "status": "success",
            "message": f"Data source '{source_row.name}' (ID: {source_id}) deleted successfully",
            "id": source_id,
            "cleaned_up": {
                "pipeline_runs_deleted": run_count,
                "datasets_deleted": dataset_count,
                "catalog_columns_deleted": column_count,
            }
        }
