...
def test_source_connection(
    ...
    if normalized_type == "salesforce":
        return SalesforceConnector.test_connection(config)

    if normalized_type in ("databricks", "postgresql"):
...
def discover_source_metadata(
    ...
    """
    Dispatcher for metadata discovery across supported platforms.
    """
    normalized_type = (source_type or "").strip().lower()
    ...
    if normalized_type == "salesforce":
        return SalesforceConnector.discover_metadata(config)
...
def execute_source_query(
    ...
    """
    Dispatcher for executing read-only queries across supported platforms.
    """
    normalized_type = (source_type or "").strip().lower()
    ...
    if normalized_type == "salesforce":
        return SalesforceConnector.execute_read_query(
            config=config,
            query=query,
            params=params,
            timeout_seconds=timeout_seconds,
