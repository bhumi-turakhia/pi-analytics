...
class SalesforceConnector:
    ...
    @classmethod
    def _get_access_token(cls, timeout_seconds: int = 15) -> Tuple[str, str]:
        login_url, client_id, client_secret = cls._settings()
        ...
        if missing:
            raise RuntimeError(
                f"Missing required Salesforce environment variable(s): {', '.join(missing)}"
            )
    ...
    def execute_read_query(
        ...
        try:
            ...
            is_overall_aggregate = bool(
                re.match(
                    r"^\s*SELECT\s+(COUNT\(\)|SUM\(|AVG\(|MIN\(|MAX\()",
                    soql,
                    re.IGNORECASE,
                )
            ) and not re.search(r"\bGROUP\s+BY\b", soql, re.IGNORECASE)

            if not is_overall_aggregate and not re.search(r"\bLIMIT\s+\d+\b", soql, re.IGNORECASE):
