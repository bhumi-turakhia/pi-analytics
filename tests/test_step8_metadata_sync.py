...
class Step8MetadataSyncTests(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        with engine.begin() as conn:
            # Delete catalog_columns and datasets created for this source
            conn.execute(
            ...
            # Delete the test data source itself
            conn.execute(
    ...
    # 18. 409 delete protection still works
    def test_18_delete_protection_409_works(self):
        # Attempting to delete a source that has referencing datasets must be blocked with 409
        with self.assertRaises(HTTPException) as ctx:
            sources.delete_source(source_id=self.test_source_id)
