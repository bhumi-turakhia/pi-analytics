...
class Step10CopilotAndDashboardTests(unittest.TestCase):
            ...
            conn.execute(
                text("""
                    ...
                    VALUES 
                        ...
                        (:ds_id, 'CUSTOMER_ID', 'VARCHAR', 6),
                        (:ds_id, 'PRODUCT_NAME', 'VARCHAR', 7)
                """),
    ...
    # 21. Natural language dates / temporal expressions semantic validation
    def test_21_natural_language_dates_semantic_validation(self):
        catalog = get_catalog_metadata(self.test_source_id)
        # Temporal phrase 'January to June 2025' must NOT be rejected as Table 'january' not found
        err = _check_unknown_references("Show sales from January to June 2025", catalog)
        self.assertIsNone(err)

        err_month = _check_unknown_references("Show monthly revenue in last quarter", catalog)
        self.assertIsNone(err_month)

        # Genuinely unknown table must STILL be rejected
        err_bad = _check_unknown_references("Show sales from secret_table", catalog)
        self.assertIsNotNone(err_bad)
        self.assertIn("secret_table", err_bad)

    # 22. Business metric grounding (revenue vs quantity)
    def test_22_business_metric_grounding_revenue_vs_quantity(self):
        catalog = get_catalog_metadata(self.test_source_id)
        # Revenue question must map to REVENUE column
        sql_rev, _ = generate_catalog_grounded_sql("Show total revenue by region", catalog)
        self.assertIn('"REVENUE"', sql_rev.upper())

        # Quantity question must map to QUANTITY column
        sql_qty, _ = generate_catalog_grounded_sql("Show total quantity by region", catalog)
        self.assertIn('"QUANTITY"', sql_qty.upper())

    # 23. Snowflake account identifier cleaning
    def test_23_snowflake_account_identifier_cleaning(self):
        from connectors.snowflake import SnowflakeConnector
        raw_url = "https://xy12345.us-east-1.snowflakecomputing.com/"
        cleaned = SnowflakeConnector.clean_account_identifier(raw_url)
        self.assertEqual(cleaned, "xy12345.us-east-1")

        raw_domain = "ab67890.snowflakecomputing.com"
        cleaned_domain = SnowflakeConnector.clean_account_identifier(raw_domain)
        self.assertEqual(cleaned_domain, "ab67890")
